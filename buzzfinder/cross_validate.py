...
impact on model performance.  It takes the raw audio clips, splits it into folds,
...
def make_loss_acc_plot(evaluations, histories):
    ...
    plt.plot(18, mean_loss, marker='o', markersize=10, label=f"test loss ({round(mean_loss * 100, 1)}%)", color='blue', alpha=0.5)
    plt.plot(18, mean_acc, marker='o', markersize=10, label=f"test accuracy ({round(mean_acc * 100, 1)}%)", color='orange')
...
if __name__ == "__main__":
    ...
    bf_cross_validate(DATASET_PATH, JSON_PATH, TYPE_OF_AUGMENTATION, AUG_PARAMETERS, cv=10)
